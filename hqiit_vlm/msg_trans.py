...
class CommandRelayNode(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            String,
            self.config['topics']['subscribers']['trans'],
            self.listener_callback,
            10)
            
        self.twist_publisher = self.create_publisher(
            Twist, 
            self.config['topics']['publishers']['cmd_vel'], 
            10)

        # 使用配置文件中的参数
        self.cmd_vel_publish_rate = self.config['cmd_vel']['publish_rate']['movement']
        self.altitude_publish_rate = self.config['cmd_vel']['publish_rate']['altitude']
        self.command_duration = self.config['cmd_vel']['duration']
        self.timeout_threshold = self.config['cmd_vel']['timeout']
        ...
        self.create_timer(0.1, self.timer_callback)  # 每0.1秒检查一次
